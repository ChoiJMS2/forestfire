...
def run_appendix():
    tab1, tab2, tab3 = st.tabs(["✅ Data List", "✅ Codebook", "✅ 부록"])
    with tab1:
        # 빅쿼리 클라이언트 객체 생성
        client = bigquery.Client(credentials=credentials, project=credentials.project_id)
        # 데이터셋 목록 조회
        st.subheader("Data Set List")
        # 데이터셋 목록 가져오기
        ...
        dataset_list = st.selectbox("Select Date Set",
                                    ('Analysis_Data', 'PreProcessing_Data', 'Raw_Data'),label_visibility='collapsed')  # 원하는 데이터셋 ID로 변경
        if dataset_list:
